...
class OrderRepo {
  ...
  Future<ApiResponse> getAllOrders() async {
    try {
      final response = await dioClient.get(
          '${AppConstants.CURRENT_ORDERS_URI}${sharedPreferences.get(AppConstants.TOKEN)}');
  ...
  Future<ApiResponse> getPendingOrders() async {
    try {
      final response = await dioClient.get(
          '${AppConstants.PENDING_ORDERS_URI}${sharedPreferences.get(AppConstants.TOKEN)}');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getOrderDetails({String orderID}) async {
    try {
      final response = await dioClient.get(
          '${AppConstants.ORDER_DETAILS_URI}${sharedPreferences.get(AppConstants.TOKEN)}&order_id=$orderID');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  //updateOrderToMe
  Future<ApiResponse> updateOrderToMe(
      {String token, int orderId, String status}) async {
    try {
      Response response = await dioClient.post(
        AppConstants.SET_ORDER_STATUS_URI,
        data: {
          "token": token,
          "order_id": orderId,
          "status": status,
